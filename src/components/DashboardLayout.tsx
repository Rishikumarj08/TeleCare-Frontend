...
export default function DashboardLayout({ title, role, navItems, children, activeNav, onNavClick }: Props) {
        ...
        <div style={{ padding: '24px 20px 12px' }}>
          ...
          <nav>
            {navItems.map((item) => (
              <button
                key={item.label}
                onClick={() => onNavClick ? onNavClick(item.label) : navigate(item.path)}
                style={{ ...navBtnStyle, background: activeNav === item.label ? 'rgba(255,255,255,0.15)' : 'transparent', color: activeNav === item.label ? '#fff' : '#bfdbfe' }}
              >
                {item.label}
              </button>
            ))}
